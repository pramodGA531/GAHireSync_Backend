...
class Organization(models.Model):
    ...
    gst_number = models.CharField(max_length=255, blank=True, default="N/A")
    ...
    manager = models.OneToOneField(
        CustomUser, on_delete=models.CASCADE, related_name="managing_organization"
    )
    recruiters = models.ManyToManyField(
        CustomUser, related_name="recruiting_organization", blank=True
    )
    # bank details
...
class ClientDetails(models.Model):
    ...
    interviewers = models.ManyToManyField(
        CustomUser, related_name="client_interviewers", blank=True
    )
...
class ClientOrganizationTerms(models.Model):
    ...
    FIXED = "fixed"
    SERVICE_TYPE_CHOICES = [(PERCENTAGE, "percentage"), (FIXED, "fixed")]

    client_organization = models.ForeignKey(
        ClientOrganizations, on_delete=models.CASCADE
    )
    ctc_range = models.CharField(default="5,6", max_length=30)
    service_fee_type = models.CharField(
        default="percentage", max_length=20, choices=SERVICE_TYPE_CHOICES
    )
...
class JobPostings(models.Model):
    ...
    description_file = models.FileField(
        upload_to="JobDescriptions/", blank=True, null=True
    )
    ...
    job_department = models.CharField(
        max_length=100,
    )
    ...
    passport_availability = models.CharField(
        max_length=50,
    )
    ...
    is_seen = models.BooleanField(default=False)
    is_edited_by_client = models.BooleanField(default=False)
...
class AssignedJobs(models.Model):
    ...
    def __str__(self):
        assigned_names = ", ".join(
            user.get_full_name() or user.username for user in self.assigned_to.all()
        )
...
class JobPostingsEditedVersion(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            latest = (
                JobPostingsEditedVersion.objects.filter(job_id=self.job_id)
    ...
    def clean(self):
        ...
        if self.field_name not in job_post_fields:
            raise ValidationError(
                f"'{self.field_name}' is not a valid field of JobPostings model."
            )
...
class SkillMetricsModel(models.Model):
    ...
    job_id = models.ForeignKey(
        JobPostings, on_delete=models.CASCADE, related_name="skills"
    )
...
class SkillMetricsModelEdited(models.Model):
    ...
    job_id = models.ForeignKey(
        JobPostingsEditedVersion, on_delete=models.CASCADE, related_name="skills"
    )
    skill_name = models.CharField(
        max_length=50,
    )
...
class InterviewerDetails(models.Model):
    ...
    name = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, limit_choices_to={"role": "interviewer"}
    )
...
class InterviewerDetailsEditedVersion(models.Model):
    ...
    mode_of_interview = models.CharField(max_length=20, choices=MODE_OF_INTERVIEW)
    type_of_interview = models.CharField(
        max_length=35, choices=InterviewerDetails.TYPE_OF_INTERVIEW
    )
...
class CandidateResume(models.Model):
    ...
    current_job_type = models.CharField(
        max_length=50,
        null=True,
        blank=True,
    ...
    highest_qualification = models.CharField(
        max_length=255, blank=True, default="B Tech"
    )
...
class CandidateSkillSet(models.Model):
    candidate = models.ForeignKey(
        CandidateResume, on_delete=models.CASCADE, related_name="skills"
    )
    is_primary = models.BooleanField(default=False)
...
class JobPostTerms(models.Model):
    ...
    service_fee_type = models.CharField(
        max_length=30,
        choices=ClientOrganizationTerms.SERVICE_TYPE_CHOICES,
        default="percentage",
    )
...
class InterviewSchedule(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    candidate = models.ForeignKey(
        CandidateResume, on_delete=models.CASCADE, blank=True, null=True
    )
...
class NegotiationRequests(models.Model):
    ...
    client_organization = models.ForeignKey(
        ClientOrganizations,
        on_delete=models.CASCADE,
        related_name="negotiation_requests",
    )
    ctc_range = models.CharField(max_length=50, null=True, blank=True)
    service_fee = models.DecimalField(max_digits=10, decimal_places=2, default=8.33)
    service_fee_type = models.CharField(
        max_length=20,
        choices=ClientOrganizationTerms.SERVICE_TYPE_CHOICES,
        default="percentage",
...
class CandidateProfile(models.Model):
    ...
    name = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="candidate_name"
    )
...
class CandidateDocuments(models.Model):
    candidate = models.ForeignKey(
        CandidateProfile, on_delete=models.CASCADE, related_name="candidate_documents"
    )
...
class CandidateCertificates(models.Model):
    candidate = models.ForeignKey(
        CandidateProfile,
        on_delete=models.CASCADE,
        related_name="candidate_certificates",
    )
...
class CandidateExperiences(models.Model):
    ...
    STATUS_CHOICES = [(WORKING, "working"), (NOTWORKING, "not_working")]

    candidate = models.ForeignKey(
        CandidateProfile, on_delete=models.CASCADE, related_name="candidate_experience"
    )
    ...
    relieving_letter = models.FileField(
        upload_to="Candidate/Experience/Leter", null=True, blank=True
    )
    pay_slip1 = models.FileField(
        upload_to="Candidate/Experience/PaySlip1/", blank=True, null=True
    )
    pay_slip2 = models.FileField(
        upload_to="Candidate/Experience/PaySlip2/", blank=True, null=True
    )
    pay_slip3 = models.FileField(
        upload_to="Candidate/Experience/PaySlip3/", blank=True, null=True
    )
...
class CandidateEducation(models.Model):
    candidate = models.ForeignKey(
        CandidateProfile, on_delete=models.CASCADE, related_name="candidate_education"
    )
...
class RecruiterProfile(models.Model):
    name = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="recruiter_profiles"
    )
    alloted_to = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="alloted_to"
    )
    organization = models.ForeignKey(
...
class SelectedCandidates(models.Model):
    ...
    application = models.OneToOneField(
        JobApplication, on_delete=models.CASCADE, related_name="selected_candidates"
    )
    ...
    joining_status = models.CharField(
        max_length=20, choices=JOINING_STATUS_CHOICES, default="pending"
    )
    candidate_acceptance = models.CharField(
    ...
    replacement_status = models.CharField(
        max_length=50, default="no", choices=REPLACEMENT_STATUS_CHOICES
    )
...
class InvoiceGenerated(models.Model):
    ...
    invoice_calculated = models.DecimalField(
        default=0.0, null=True, max_digits=30, decimal_places=3
    )
    ...
    cgst = models.DecimalField(
        default=0.0, null=True, blank=True, max_digits=30, decimal_places=3
    )
    ...
    payment_transaction_id = models.CharField(
        null=True, blank=True, default="null", max_length=50
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            self.invoice_code = (
                f"{self.organization.org_code}-{self.client.id}-{unique_suffix}"
            )
...
class Accountants(models.Model):
    ...
    user = models.OneToOneField(
        CustomUser, on_delete=models.CASCADE, related_name="accountant"
    )
    email = models.EmailField(unique=True)
...
class ReplacementCandidates(models.Model):
    replacement_with = models.OneToOneField(
        JobApplication,
        on_delete=models.CASCADE,
        related_name="replacement_with",
        limit_choices_to={"status": "left"},
    )
...
class Tickets(models.Model):
    ...
    raised_by = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="tickets_raised"
    )
...
class Messages(models.Model):
    ...
    attachment = models.FileField(
        upload_to="ticket_attachements/", null=True, blank=True
    )
...
class BlogImage(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="blog_images"
    )
...
class Notifications(models.Model):
    ...
    category = models.CharField(
        max_length=60, choices=CategoryChoices.choices, null=True, blank=True
    )
...
class JobPostingDraftVersion(models.Model):
    ...
    assigned_to = models.ManyToManyField(
        CustomUser,
        related_name="assigned_jobs_draft",
        limit_choices_to={"role": "recruiter"},
        blank=True,
    )
...
class JobLocationsDraftVersion(models.Model):
    ...
    job = models.ForeignKey(
        JobPostingDraftVersion, on_delete=models.CASCADE, related_name="locations"
    )
...
class SkillMetricsDraftVersion(models.Model):
    job = models.ForeignKey(
        JobPostingDraftVersion, on_delete=models.CASCADE, related_name="skill_metrics"
    )
...
class InterviewerDetailsDraftVersion(models.Model):
    ...
    job = models.ForeignKey(
        JobPostingDraftVersion, on_delete=models.CASCADE, related_name="interviewers"
    )
    ...
    mode_of_interview = models.CharField(
        max_length=20, choices=MODE_OF_INTERVIEW, blank=True
    )
    type_of_interview = models.CharField(
        max_length=35, choices=TYPE_OF_INTERVIEW, blank=True
    )
...
class OrganizationPlan(models.Model):
    ...
    plan = models.ForeignKey(
        Plan, on_delete=models.SET_NULL, null=True, related_name="organization_plans"
    )
    ...
    is_active = models.BooleanField(
        default=True
    )  # Marks if this is the current active plan
...
class PlanHistory(models.Model):
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="plan_history"
    )
...
class JobPostLog(models.Model):
    job_post = models.ForeignKey(
        JobPostings, on_delete=models.CASCADE, related_name="logs"
    )
    message = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
...
class InterviewLog(models.Model):
    interview = models.ForeignKey(
        InterviewSchedule, on_delete=models.CASCADE, related_name="logs"
    )
    message = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
...
class JobEditRequestsByClient(models.Model):
    id = models.AutoField(primary_key=True)
    job_id = models.ForeignKey(JobPostings, on_delete=models.CASCADE)
    ...
    job_department = models.CharField(max_length=100, null=True, blank=True)
    job_description = models.TextField(null=True, blank=True)
    years_of_experience = models.TextField(max_length=100, null=True, blank=True)
    ctc = models.CharField(max_length=50, null=True, blank=True)
    rounds_of_interview = models.IntegerField(null=True, blank=True)
    ...
    decision_maker_email = models.CharField(max_length=100, blank=True)
    bond = models.TextField(max_length=255, blank=True)
    rotational_shift = models.BooleanField(null=True, blank=True)
    status = models.CharField(max_length=10, default="opened", null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True, null=True)
    age = models.CharField(max_length=255, null=True, blank=True)
    gender = models.CharField(max_length=30, blank=True, null=True)
    visa_status = models.CharField(max_length=100, null=True, blank=True)
    passport_availability = models.CharField(max_length=50, null=True, blank=True)
    time_period = models.CharField(max_length=50, default=" ", blank=True)
    ...
    differently_abled = models.CharField(max_length=40, blank=True, null=True)
    ...
    is_linkedin_posted = models.BooleanField(default=False)
    is_seen = models.BooleanField(default=False)
    job_locations = models.JSONField(default=list, null=True, blank=True)
