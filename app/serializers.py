...
class JobPostingsSerializer(serializers.ModelSerializer):
    ...
    locations = LocationSerializer(
        many=True, read_only=True, source="joblocationsmodel_set"
    )
    interview_details = InterviewerDetailsSerializer(
        many=True, read_only=True, source="interviewerdetails_set"
    )
    ...
    def get_selectedCandidatesCount(self, obj):
        return SelectedCandidates.objects.filter(
            application__job_location__job_id=obj
        ).count()

    def get_onHoldCount(self, obj):
        return SelectedCandidates.objects.filter(
            application__status="onHold", application__job_location__job_id=obj
        ).count()

    def get_rejectedCount(self, obj):
        return SelectedCandidates.objects.filter(
            application__status="rejected", application__job_location__job_id=obj
        ).count()

    def get_pendingCount(self, obj):
        return SelectedCandidates.objects.filter(
            application__status="pending", application__job_location__job_id=obj
        ).count()
...
class JobPostEditedSerializer(serializers.ModelSerializer):
    interview_details = InterviewDetailsEditedSerializer(
        many=True, read_only=True, source="interviewerdetailseditedversion_set"
    )
        ...
        if isinstance(custom_user_data, CustomUser):
            ...
            try:
                ...
                raise serializers.ValidationError(
                    {"name": "Invalid user ID. User does not exist."}
                )
...
class JobInterviewerDetailsSerializer(serializers.ModelSerializer):
    user_id = serializers.IntegerField(source="name.id", read_only=True)
