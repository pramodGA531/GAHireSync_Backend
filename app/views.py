...
class OrganizationTermsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if not organization:
                return Response(
                    {"error": "Organization doesnot found"},
            ...
            for connection in connections:
                connection_terms = ClientOrganizationTerms.objects.filter(
                ...
                try:
                    ...
                    logger.error(
                        f"Error fetching negotiated terms for connection {connection.id}: {neg_e}"
                    )
                all_terms.append(
                    {
    ...
    def post(self, request):
        try:
            ...
            data = request.data
            organization_total_terms = OrganizationTerms.objects.filter(
            ...
            if unique_code == "" or unique_code == None:
                unique_code = self.generate_unique_code(
                    organization.org_code, organization_total_terms
                )
            ...
            return Response(
                {
                    "message": "Organization terms added successfully",
                    "id": new_terms.id,
                },
            ...
            if "unique_code" in str(e):
                return Response(
                    {"error": "unique_code already exists"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            return Response(
    ...
    def put(self, request):
        try:
            ...
            organization = Organization.objects.filter(manager=user).first()
            data = request.data
            organization_terms = get_object_or_404(
                OrganizationTerms, organization=organization, id=request.data.get("id")
            )
            if data.get("description"):
                organization_terms.description = data.get(
                ...
                organization_terms.unique_code = data.get(
                    "unique_code", organization_terms.unique_code
                )
                organization_terms.service_fee = data.get(
                    "service_fee", organization_terms.service_fee
                )
                organization_terms.replacement_clause = data.get(
                    "replacement_clause", organization_terms.replacement_clause
                )
                organization_terms.invoice_after = data.get(
                    "invoice_after", organization_terms.invoice_after
                )
                organization_terms.payment_within = data.get(
                    "payment_within", organization_terms.payment_within
                )
                organization_terms.interest_percentage = data.get(
                ...
                return Response(
                    {
                        "detail": "Organization terms updated successfully",
                        "id": organization_terms.id,
                    },
                    status=status.HTTP_200_OK,
                )
        except Exception as e:
...
class NegotiateTermsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.role == "manager":
            ...
            negotiationrequests = NegotiationRequests.objects.filter(
                client_organization__organization=organization, status="pending"
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.role != "client":
            return Response(
                {"detail": "Only clients can create negotiation requests"},
        ...
        if not connection:
            return Response(
                {"detail": "Invalid connection id"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            prev_negotiations = NegotiationRequests.objects.get(
                client_organization=connection
            )
...
"""
            ...
            return Response(
                {"detail": "Negotiation request created successfully"},
                status=status.HTTP_201_CREATED,
            )
    ...
    def put(self, request):
        ...
        if user.role != "manager":
            return Response(
                {"detail": "Only managers can update negotiation requests"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            ...
            if data.get("status") == "accepted":
                ...
                negotiation_request.expiry_date = datetime.today().date() + timedelta(
                    days=365
                )
...
HireSync Team
                ...
                send_custom_mail(
                    ...
                    to_email=[
                        negotiation_request.client_organization.client.user.email
                    ],
                )
...
"""
                ...
                return Response(
                    {
                        "detail": "Invalid status provided. Please use 'accepted' or 'rejected'."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )

            return Response(
                {"detail": "Negotiation request updated successfully"},
                status=status.HTTP_200_OK,
            )
...
class JobDetailsAPIView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            assigned_recruiters = AssignedJobs.objects.filter(
            ...
            for assignment in assigned_recruiters:
                ...
                recruiter_ids = list(
                    assignment.assigned_to.values_list("id", flat=True)
                )
            ...
            # Notification for Manager viewing job for the first time
            if not job_posting.is_seen and request.user.role == CustomUser.MANAGER:
                job_posting.is_seen = True
                job_posting.save()

                # Create Dashboard Notification
                Notifications.objects.create(
                    sender=request.user,
                    receiver=job_posting.username,
                    category=Notifications.CategoryChoices.MANAGER_VIEWED_JOB,
                    subject=f"Job description viewed by manager: {job_posting.job_title}",
                    message=(
                        f"Dear {job_posting.username.username},\n\n"
                        f"The manager {request.user.username} has viewed your job posting for '{job_posting.job_title}'.\n\n"
                        f"They are now reviewing the details to proceed with recruitment.\n\n"
                        f"Best regards,\n"
                        f"HireSync Team"
                    ),
                )
                ...
                # Send Email
                email_subject = (
                    f"Manager is reviewing your job: {job_posting.job_title}"
                )
                email_body = (
                    f"Dear {job_posting.username.username},\n\n"
                    f"We are pleased to inform you that the manager {request.user.username} has viewed "
                    f"the job description for **{job_posting.job_title}**.\n\n"
                    f"This indicates that your job post is under active review.\n\n"
                    f"Best regards,\n"
                    f"HireSync Team"
                )
                send_custom_mail(
                    ...
                    body=email_body,
                    to_email=[job_posting.username.email],
                )

            return Response(
                {"job": data, "assigned_recruiters": assigned_recruiters_map},
                status=status.HTTP_200_OK,
            )

        except JobPostings.DoesNotExist:
            return Response(
                {"detail": "Job posting not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if not user and not recruiter_id:
                return Response(
                    {"detail": "User or recruiter ID is required"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            job_postings = JobPostings.objects.filter(
                organization=org, assigned_to=assigned_user
            )
...
class ViewCandidateProfileAPI(APIView):
    def get(self, request):
        try:
            ...
            candidate_documents = CandidateDocuments.objects.filter(
                candidate=candidate_profile
            )
        ...
        try:
            ...
            is_new_position = (
                not JobPostings.objects.filter(
                    job_title=job_details.job_title, organization__manager=request.user
                )
                .exclude(id=job_id)
                .exists()
            )
            ...
            if job_application.status == "rejected":
                ...
                selected_round = (
                    f"Round-{job_application.round_num}"
                    if job_application.round_num
                    else "Not Assigned"
                )
            ...
            interview_data = {
                "next_round_time": (
                    next_interview.schedule_date if next_interview else None
                ),
                "interviewer": (
                    next_interview.interviewer.name.username if next_interview else None
                ),
                "meet_link": next_interview.meet_link if next_interview else None,
                "interview_type": (
                    next_interview.interviewer.type_of_interview
                    if next_interview
                    else None
                ),
                "interview_mode": (
                    next_interview.interviewer.mode_of_interview
                    if next_interview
                    else None
                ),
            }
            ...
            response_data = {
                ...
                "recruiter_name": (
                    job_application.attached_to.username
                    if job_application.attached_to
                    else None
                ),
                "num_of_rounds": InterviewerDetails.objects.filter(
                    job_id=job_details
                ).count(),
...
class NotificationToUpdateProfileView(APIView):
    def post(self, request):
        try:
            ...
            if not candidate_id:
                return Response(
                    {"error": "Candidate Id is not sent"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Candidate Profile Does Not Exists"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            message = """
...
"""
            ...
            try:
                send_custom_mail(
                    subject=subject, body=message, to_email=[candidate_email]
                )
    ...
    def get(self, request):
        try:
            ...
            if request.user.role == "client":
                ...
                invoices = InvoiceGenerated.objects.filter(
                    organization__manager__email=request.user.email
                )
                for invoice in invoices:
                    context = create_invoice_context(invoice.id)
                    html = generate_invoice(context)
                    html_list.append({"invoice": invoice, "html": html})
                accountants = Accountants.objects.filter(
                    ...
                    invoice_data = [
                        {
                            ...
                            "payment_status": invoice.payment_status,
                            "client_email": invoice.client.user.email,
                            "org_email": invoice.organization.manager.email,
                            "html": html["html"],
                            "payment_verification": invoice.payment_verification,
                        }
                ...
                return Response(
                    {"invoices": invoice_data, "accountants": serializer.data},
                    status=status.HTTP_200_OK,
                )
    ...
    def put(self, request):
        ...
        if request.user.role == "client":
            ...
            if not payment_transaction_id or not payment_method:
                return Response(
                    {
                        "error": "Both payment_transaction_id and payment_method are required."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            invoice.save()
            return Response(
                {"message": "Invoice marked as paid by client."},
                status=status.HTTP_200_OK,
            )
            ...
            if payment_verification is None:
                return Response(
                    {"error": "payment_verification field is required."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            invoice.save()
            return Response(
                {"message": "Invoice payment verified by accountant."},
                status=status.HTTP_200_OK,
            )
    ...
    def get(self, request, application_id):
        try:
            ...
            if not resume_details:
                return Response(
                    {"error": "No resume details associated with this application"},
                    status=status.HTTP_404_NOT_FOUND,
                )
...
class RaiseTicketView(APIView):
                        ...
                        replies_list.append(
                            {
                                ...
                                "name": (
                                    reply.ticket_id.raised_by.username
                                    if reply.is_user_raised_by
                                    else reply.ticket_id.assigned_to.username
                                ),
                                "message": reply.message,
                                "attachment": (
                                    reply.attachment.url if reply.attachment else None
                                ),
                                "attachment_name": (
                                    reply.attachment.name if reply.attachment else None
                                ),
                    ...
                    return Response(
                        {
                            ...
                            "attachments": (
                                ticket.attachments.url if ticket.attachments else None
                            ),
                            "attachment_name": (
                                ticket.attachments.name if ticket.attachments else None
                            ),
                        },
                        status=status.HTTP_200_OK,
                    )
    ...
    def post(self, request):
        try:
            ...
            # Notify Admin (Dashboard + Email)
            admin = new_ticket.assigned_to
            Notifications.objects.create(
                sender=user,
                receiver=admin,
                category=Notifications.CategoryChoices.TICKET_RAISED,
                subject=f"New Support Ticket: {new_ticket.category}",
                message=(
                    f"A new support ticket has been raised by {user.username}.\n\n"
                    f"Category: {new_ticket.category}\n"
                    f"Priority: {new_ticket.priority}\n"
                    f"Description: {new_ticket.description[:100]}..."
                ),
            )
            email_body = f"Hello {admin.username},\n\nA new support ticket has been raised by {user.username}.\n\nCategory: {new_ticket.category}\nPriority: {new_ticket.priority}\n\nPlease review it in the admin dashboard."
            send_custom_mail(
                subject=f"New Support Ticket Raised - {new_ticket.category}",
                to_email=[admin.email],
                body=email_body,
            )

            return Response(
                {"message": "Ticket Raised successfully"}, status=status.HTTP_200_OK
            )
...
class UpdateStatus(APIView):
    def put(self, request):
        try:
            ...
            Notifications.objects.create(
                sender=request.user,
                receiver=ticket.raised_by,
                category=Notifications.CategoryChoices.TICKET_STATUS_UPDATE,
                subject=f"Ticket Resolved: {ticket.category}",
                message=f"Your ticket regarding '{ticket.category}' has been marked as completed.",
            )
            email_body = f"Hello {ticket.raised_by.username},\n\nYour ticket regarding '{ticket.category}' has been marked as completed/resolved.\n\nThank you for reaching out."
            send_custom_mail(
                subject=f"Ticket Resolved - {ticket.category}",
                to_email=[ticket.raised_by.email],
                body=email_body,
            )
...
class HandleReplies(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Notify Other Party (Dashboard + Email)
            receiver = (
                ticket.assigned_to if ticket.raised_by == user else ticket.raised_by
            )
            Notifications.objects.create(
                sender=user,
                receiver=receiver,
                category=Notifications.CategoryChoices.TICKET_REPLY,
                subject=f"New Reply on Ticket: {ticket.category}",
                message=f"You have a new reply from {user.username} on ticket #{ticket.id}.",
            )
            email_body = f"Hello {receiver.username},\n\nYou have received a new reply from {user.username} on your support ticket regarding '{ticket.category}'.\n\nMessage: {reply.message[:100]}..."
            send_custom_mail(
                subject=f"New Ticket Reply - {ticket.category}",
                to_email=[receiver.email],
                body=email_body,
            )

            return Response(
                {"message": "Response saved successfully"}, status=status.HTTP_200_OK
            )
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Ticket does not exist"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            if old_status != ticket.status and request.user != ticket.raised_by:
                Notifications.objects.create(
                    sender=request.user,
                    receiver=ticket.raised_by,
                    category=Notifications.CategoryChoices.TICKET_STATUS_UPDATE,
                    subject=f"Ticket Status Updated: {ticket.category}",
                    message=f"Your ticket status has been updated to '{ticket.status}'.",
                )
                email_body = f"Hello {ticket.raised_by.username},\n\nYour ticket regarding '{ticket.category}' has been updated to status: {ticket.status}.\n\nBest regards,\nHireSync Team"
                send_custom_mail(
                    subject=f"Ticket Status Update - {ticket.category}",
                    to_email=[ticket.raised_by.email],
                    body=email_body,
                )

            return Response(
                {"message": "Reply sent successfully"}, status=status.HTTP_200_OK
            )
        ...
        if not is_approved:
            ...
            for admin in admins:
                Notifications.objects.create(
                    sender=user,
                    receiver=admin,
                    category=Notifications.CategoryChoices.BLOG_SUBMITTED,
                    subject=f"New Blog Submitted: {title}",
                    message=f"Author {user.username} has submitted a new blog post '{title}' for approval.",
                )
                email_body = f"Hello {admin.username},\n\nA new blog post '{title}' has been submitted by {user.username} and is awaiting your approval.\n\nPlease check the admin panel."
                send_custom_mail(
                    subject=f"New Blog Submission - {title}",
                    to_email=[admin.email],
                    body=email_body,
                )
    ...
    def get(self, request):
        try:
            ...
            if not user.role == "admin":
                return Response(
                    {"error": "Your are not allowed to run this view"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Cant process the request, blog post not available"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            Notifications.objects.create(
                sender=request.user,
                receiver=blog.user,
                category=Notifications.CategoryChoices.BLOG_APPROVED,
                subject=f"Blog Approved: {blog.title}",
                message=f"Congratulations! Your blog post '{blog.title}' has been approved and is now live.",
            )
            email_body = f"Hello {blog.user.username},\n\nYour blog post '{blog.title}' has been approved by the admin and is now live on the platform.\n\nCongratulations!"
            send_custom_mail(
                subject=f"Blog Approved - {blog.title}",
                to_email=[blog.user.email],
                body=email_body,
            )
...
class GetJobPostTerms(APIView):
    ...
    def get(self, request):
        try:
            ...
            jobs = JobPostings.objects.filter(username=client).select_related(
                "organization"
            )
    ...
    def get(self, request):
        try:
            ...
            if count_param is not None:
                unseen_notifications_count = Notifications.objects.filter(
                    receiver=request.user, seen=False
                ).count()
                return Response(
                    {"count": unseen_notifications_count}, status=status.HTTP_200_OK
                )
    ...
    def put(self, request):
        try:
            ...
            if notifications.exists():
                ...
                return Response(
                    {"data": "Successfully updated"}, status=status.HTTP_200_OK
                )
            else:
                return Response(
...
class MarkAllNotificationsAsSeenView(APIView):
    def patch(self, request):
        try:
            Notifications.objects.filter(receiver=request.user, seen=False).update(
                seen=True
            )
            return Response(
        ...
        try:
            ...
            if recruiter_id:
                ...
                applications = JobApplication.objects.filter(
                    job_location__job_id=job.id, attached_to=recruiter
                )
            else:
                applications = JobApplication.objects.filter(
    ...
    def put(self, request):
        try:
            ...
            if type(category) == list:
                notifications = Notifications.objects.filter(
                    receiver=user, seen=False, category__in=category
                )
            else:
                notifications = Notifications.objects.filter(
                    receiver=user, seen=False, category=category
                )
...
class CompleteApplicationDetailsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            primary_skills = CandidateSkillSet.objects.filter(
                candidate=application.resume, is_primary=True
            )
            primary_skill_json = []
            ...
            secondary_skills = CandidateSkillSet.objects.filter(
                candidate=application.resume, is_primary=False
            )
            ...
            for candidate in candidate_evaluations:
                candidate_evaluation_json[candidate.round_num].append(
                    {
                        "primary_skills_rating": json.dumps(
                            candidate.primary_skills_rating
                        ),
                        "secondary_skills_rating": json.dumps(
                            candidate.secondary_skills_ratings
                        ),
            ...
            application_json = {
                ...
                "resume": (
                    ...
                    if application.resume.resume != None
                    else ""
                ),
            ...
            return Response(
                {"application_data": application_json}, status=status.HTTP_200_OK
            )
...
class LinkedInRedirectView(APIView):
    def get(self, request):
        try:
            ...
            redirect_url = (
                f"http://localhost:8000/hiresync/generate-linkedincode/callback/"
            )
            ...
            if not code:
                return Response(
                    {"message": "Missing authorization code.", "status": False},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class FetchAllJobs(APIView):
    def get(self, request):
        try:
            ...
            for job in jobs:
                locations = list(
                    JobLocationsModel.objects.filter(job_id=job.id).values_list(
                        "location", flat=True
                    )
                )
                jobs_list.append(
...
class SendApplicationDetailsView(APIView):
    def post(self, request):
        try:
            ...
            location_instance = JobLocationsModel.objects.get(
                job_id=job_id, location=location
            )

            if not job_id:
                return Response(
                    {"error": "Jobid is required"}, status=status.HTTP_400_BAD_REQUEST
                )

            email = data.get("candidate_email")

            if JobApplication.objects.filter(
                job_location__job_id__id=job_id, resume__candidate_email=email
            ).exists():
                return Response(
                    {
            ...
            return Response(
                {"message": "Application sent successfully"}, status=status.HTTP_200_OK
            )
    ...
    def put(self, request):
        try:
            ...
            job_profile_log(
                ...
                f"Joining date updated to {joining_date} for candidate {selected_candidate.candidate.name}",
            )
            Notifications.objects.create(
                receiver=selected_candidate.candidate.name,
                sender=request.user,
                category=Notifications.CategoryChoices.CANDIDATE_JOINING_DATE_UPDATE,
                subject=f"Joining date updated for {selected_candidate.candidate.name}",
                message=f"Joining date updated to {joining_date} for candidate {selected_candidate.candidate.name}",
                # type="info",
            )
    ...
    def put(self, request):
        try:
            ...
            if not selected_candidate_id:
                return Response(
                    {"error": "Missing candidate_id in query params."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            selected_candidate.left_reason = (
                "Candidate did not join on the joining date"
            )
            ...
            if str(want_new_candidate).lower() == "true":
                ...
                return Response(
                    {"message": "Job reopened successfully."}, status=status.HTTP_200_OK
                )

            return Response(
                {"message": "Candidate left status updated successfully."},
                status=status.HTTP_200_OK,
            )
            ...
            for plan in plans:
                ...
                for pf in plan_features:
                    plan_data["features"].append(
                        {"id": pf.id, "feature": pf.feature.name, "limit": pf.limit}
                    )
    ...
    def get(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Client details not found for the user."},
                    status=status.HTTP_404_NOT_FOUND,
                )
...
"""
            ...
            return Response(
                {"message": "Plan upgradation request sent successfully"},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class BlogImageUploadView(APIView):
    ...
    def post(self, request):
        ...
        image_instances = []
        for file in files:
            image_obj = BlogImage.objects.create(image=file, uploaded_by=request.user)
        ...
        return Response(
            {"urls": urls, "images": serializer.data}, status=status.HTTP_201_CREATED
        )
