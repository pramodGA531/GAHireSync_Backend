    ...
    def get(self, request):
        ...
        try:
            ...
            application_counts_dict = {
                item["job_location__job_id"]: item["total"]
            ...
            for post in job_posts:
                ...
                edit_request = JobPostingsEditedVersion.objects.filter(
                    job_id=post.id
                ).first()
                if edit_request:
                ...
                jobs_list.append(
                    {
                        ...
                        "applications_last_week": recent_applications_dict.get(
                            post.id, 0
                        ),
            ...
            interviewers = ClientDetails.objects.get(
                user=request.user
            ).interviewers.all()

            interviewer_ids = [interviewer.id for interviewer in interviewers]
            interviewer_usernames = {
                interviewer.id: interviewer.username for interviewer in interviewers
            }
            interviewer_emails = {
            ...
            rounds_status = InterviewSchedule.objects.filter(
                interviewer_id__name__in=interviewer_ids
            ).values("interviewer_id__name", "status")
            ...
            for round_status in rounds_status:
                if round_status["status"] == "pending":
                    rounds_status_count[round_status["interviewer_id__name"]][
                        "pending"
                    ] += 1
            ...
            print(str(e))
            return Response(
                {"error": f"Failed to create job post terms: {str(e)}"},
...
class GetOrganizationTermsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            isDraftExists = False
            try:
                draft = JobPostingDraftVersion.objects.get(
                    username=request.user, organization=connection.organization
                )
                isDraftExists = True
...
class JobPostingView(APIView):
    ...
    def addTermsAndConditions(self, job_post, connection_id):
        ...
        try:
            ...
            for terms in client_organization_terms:
                print(
                    terms.service_fee,
                    terms.interest_percentage,
                    type(terms.service_fee),
                    type(terms.interest_percentage),
                )
            ...
            if negotiated_terms:
                ...
                JobPostTerms.objects.create(
                    ...
                    invoice_after=negotiated_terms.invoice_after,
                    payment_within=negotiated_terms.payment_within,
                    interest_percentage=safe_decimal(
                        negotiated_terms.interest_percentage
                    ),
                    is_negotiated=True,
                    ctc_range=negotiated_terms.ctc_range,
                )
            ...
            return Response(
                {"message": "Job post terms added successfully"},
    ...
    def post(self, request):
        ...
        if not generated_job_code:
            return Response(
                {"error": "Failed to generate job code"},
        ...
        elif request.data.get("description_file") and isinstance(
            request.data.get("description_file"), str
        ):
...
"""
                ...
                job_post_log(
                    job_posting.id,
                    f"client {request.user.username} has sent a new job request to organization {organization.name} orgcode:{organization.org_code} job position is {job_posting.job_title}",
                )

            return Response(
                {
                    "message": "Job and interview rounds created successfully",
    ...
    def put(self, request, job_id):
        ...
        if job_posting.username != request.user:
            return Response(
                {"error": "You do not have permission to edit this job posting."},
                status=status.HTTP_403_FORBIDDEN,
            )

        job_posting.job_title = data.get("job_title", job_posting.job_title)
        job_posting.job_department = data.get(
            "job_department", job_posting.job_department
        )
        job_posting.job_description = data.get(
            "job_description", job_posting.job_description
        )
        job_posting.primary_skills = data.get(
            "primary_skills", job_posting.primary_skills
        )
        job_posting.secondary_skills = data.get(
            "secondary_skills", job_posting.secondary_skills
        )
        job_posting.years_of_experience = data.get(
            "years_of_experience", job_posting.years_of_experience
        )
        job_posting.ctc = data.get("ctc", job_posting.ctc)
        job_posting.rounds_of_interview = data.get(
            "rounds_of_interview", job_posting.rounds_of_interview
        )
        ...
        job_posting.qualifications = data.get(
            "qualifications", job_posting.qualifications
        )
        job_posting.timings = data.get("timings", job_posting.timings)
        job_posting.other_benefits = data.get(
            "other_benefits", job_posting.other_benefits
        )
        job_posting.working_days_per_week = data.get(
            "working_days_per_week", job_posting.working_days_per_week
        )
        job_posting.decision_maker = data.get(
            "decision_maker", job_posting.decision_maker
        )
        job_posting.decision_maker_email = data.get(
            "decision_maker_email", job_posting.decision_maker_email
        )
        job_posting.bond = data.get("bond", job_posting.bond)
        job_posting.rotational_shift = data.get(
            "rotational_shift", job_posting.rotational_shift
        )
        ...
        job_posting.differently_abled = data.get(
            "differently_abled", job_posting.differently_abled
        )
        job_posting.visa_status = data.get("visa_status", job_posting.visa_status)

        job_posting.probation_type = data.get(
            "probation_type", job_posting.probation_type
        )
        job_posting.passport_availability = data.get(
            "passport_availability", job_posting.passport_availability
        )
        ...
        job_posting.num_of_positions = data.get(
            "num_of_positions", job_posting.num_of_positions
        )
        job_posting.job_close_duration = data.get(
            "job_close_duration", job_posting.job_close_duration
        )

        job_posting.save()

        return Response(
            {"message": "Job posting updated successfully", "id": job_posting.id},
            status=status.HTTP_200_OK,
        )
        ...
        try:
            ...
            if request.GET.get("id"):
                ...
                try:
                    ...
                    return Response(
                        {"edited_job": edited_data_json, "job": serialized_job.data},
                        status=status.HTTP_200_OK,
                    )
                    ...
                    return Response(
                        {"error": "Job posting edited not found"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if not edited_jobs:
                    return Response(
                        {"details": "There are no Edit Job Requests"},
                        status=status.HTTP_200_OK,
                    )
                jobs_list = []
...
class AcceptJobEditRequestView(APIView):
    ...
    def post(self, request):
        try:
            ...
            edited_field_values = JobPostEditFields.objects.filter(
                edit_id=edited_fields_version
            )
                    ...
                    if field_edit.field_name in accepted_field_names:
                        ...
                        new_value = next(
                            (
                                item["field_value"]
                                for item in changes
                                if item["field_name"] == field_edit.field_name
                            ),
                            None,
                        )
...
"""
            ...
            job_post_log(
                job_post.id,
                f"The client has reviewed and taken action on organizaion job edit  request for the position: {job_post.job_title}",
            )

            return Response(
                {"message": "Job edit request accepted successfully"},
                status=status.HTTP_200_OK,
            )

        except JobPostings.DoesNotExist:
            return Response(
                {"error": "Job post not found"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
"""
            ...
            return Response(
                {"error": "Edited Job not found"}, status=status.HTTP_400_BAD_REQUEST
            )
        except JobPostingsEditedVersion.DoesNotExist:
            return Response(
                {"error": "Edited Job not found"}, status=status.HTTP_400_BAD_REQUEST
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for interviewer in interviewers:
                ...
                scheduled_interviews = InterviewSchedule.objects.filter(
                    interviewer__in=rounds_alloted
                )
                scheduled_count = scheduled_interviews.count()
                rounds_completed = scheduled_interviews.filter(
                    status="completed"
                ).count()
                interviewer_json = {
        ...
        try:
            ...
            if user_serializer.is_valid(raise_exception=True):
                ...
                send_email_verification_link(
                    new_user, True, "interviewer", password=password
                )
    ...
    def delete(self, request):
        try:
            ...
            if not interviewer_id:
                return Response(
                    {"error": "interviewer_id is required in query params."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if data:
                for job in data:
                    ...
                    interviewer_details = InterviewerDetails.objects.get(
                        round_num=round_num, job_id=job_id
                    )
            ...
            return Response(
                {"error": "Interviewer details not found for some jobs."},
                status=status.HTTP_404_NOT_FOUND,
            )
        except ClientDetails.DoesNotExist:
            return Response(
...
class GetResumeView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if user.role != "client":
                return Response(
                    {"error": "User client is only allowed to do this job"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def post(self, request):
        try:
            if not request.data.get("feedback"):
                return Response(
                    {"error": "Feedback is not sent, please give the feedback"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not id:
                return Response(
                    {"error": "Application Id is mandatory to reject the application"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"error": "Candidate Resume not exists with that id"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        except JobApplication.DoesNotExist:
            return Response(
                {"error": "Job Application does not exists"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request):
        try:
            ...
            with transaction.atomic():
                ...
                job_application.feedback = request.data.get("feedback")
                candidate, customCand, password = self.create_user_and_profile(
    ...
    def post(self, request):
        try:
            ...
            with transaction.atomic():
                ...
                candidate, customCand = self.create_user_and_profile(
                    candidate_email=resume.candidate_email,
                    candidate_name=resume.candidate_name,
                )
                job_application.save()
    ...
    def get(self, request):
        try:
            ...
            if job.username != request.user:
                return Response(
                    {"error": "You are not authorized to view this job posting"},
                    status=status.HTTP_403_FORBIDDEN,
                )
    ...
    def get(self, request):
        try:
            ...
            if job_id:
                ...
                client = request.user
                job_ids = JobPostings.objects.filter(username=client).values_list(
                    "id", flat=True
                )
            ...
            return Response(
                {"scheduled_interviews": interviews_list, "interviewers": rounds_list},
                status=status.HTTP_200_OK,
            )
    ...
    def get(self, request):
        try:
            ...
            if job_id and job_id.isdigit():
                location_ids = list(
                    JobLocationsModel.objects.filter(job_id=job_id).values_list(
                        "id", flat=True
                    )
                )
...
class HandleSelect(APIView):
    ...
    def post(self, request):
        try:
            ...
            if (
                selected_applications >= application.job_location.positions
                or application.job_location.status == "closed"
            ):
                return Response(
                    {
                        "error": "All Job Postings are filled, If you want to recruit extra members recruit renew your job post with locations"
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class ClosedJobsClient(APIView):
    ...
    def get(self, request):
        try:
            closed_jobs = JobPostings.objects.filter(
                username=request.user,
            )
            closed_jobs_list = []
    ...
    def post(self, request):
        try:
            ...
            if user != job_post.username:
                return Response(
                    {"error": "usernames are not matching"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            JobApplication.objects.filter(
                job_id=job_post.id, status__in=["processing", "pending"]
        ...
        try:
            ...
            with transaction.atomic():
                try:
                    ...
                    return Response(
                        {"error": "Database integrity error while creating job post"},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    )
                except ValidationError as e:
                    return Response(
                        {"error": f"Invalid job post data: {str(e)}"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                actual_secondary_skills = SkillMetricsModel.objects.filter(
                    job_id=job_id, is_primary=False
                )
                ...
                if not isinstance(interviewer_details, list):
                    return Response(
                        {"error": "Invalid format for interviewer details"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )

                for interviewe in interviewer_details:
                    try:
                        interviewer = CustomUser.objects.get(
                            id=interviewe.get("interviewer_id")
                        )
                    except CustomUser.DoesNotExist:
                        return Response(
                            {
                                "error": f"Interviewer with ID {interviewe.get('interviewer_id')} not found"
                            },
                            status=status.HTTP_404_NOT_FOUND,
                        )
                    ...
                    try:
                        ...
                        return Response(
                            {"error": f"Invalid interviewer data: {str(e)}"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )

            return Response(
                {
                    "message": "Job Post Renewed successfully",
                    "new_job_id": new_job_post.id,
                },
                status=status.HTTP_201_CREATED,
            )
...
class UpdateJoiningStatus(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            job.status = "closed"
            job.save()
            remaining_applications = JobApplication.objects.exclude(
                status="selected"
            ...
            return Response(
                {
                    "message": "All positions are filled for this job posting successfully, Job posting is closed"
                },
                status=status.HTTP_200_OK,
            )
...
class AllSelectedCandidates(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"error": "No job applications found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def get(self, request):
        try:
            ...
            if not candidates_list:
                return Response(
                    {"message": "No joined candidates found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class CandidateLeftView(APIView):
    ...
    def get(self, request):
        try:
            ...
            for application in applications:
                selected_candidate = SelectedCandidates.objects.filter(
                    application=application
                ).first()
    ...
    def post(self, request):
        try:
            ...
            new_application = JobApplication.objects.get(
                id=data.get("new_application_id")
            )
    ...
    def post(self, request):
        try:
            ...
            if not location_id or not application_ids:
                return Response(
                    {"error": "Both jobid and application_ids are required."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            applications = JobApplication.objects.filter(
                id__in=application_ids, job_location=location_id
            )
            application_list = []
            ...
            for application in applications:
                ...
                primary_skills_qs = CandidateSkillSet.objects.filter(
                    candidate=resume, is_primary=True
                )
                secondary_skills_qs = CandidateSkillSet.objects.filter(
                    candidate=resume, is_primary=False
                )
            ...
            return Response(
                {"data": application_list, "job_data": job_data},
                status=status.HTTP_200_OK,
            )
    ...
    def get(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Job application with that id does not exist"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def get(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Candidate with this id doesnot exists"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            candidate_experiences_qs = CandidateExperiences.objects.filter(
                candidate=candidate_profile.id
            )
            candidate_education_qs = CandidateEducation.objects.filter(
                candidate=candidate_profile.id,
            )

            candidate_evaluation = CandidateEvaluation.objects.filter(
            ...
            candidate_details_json = {
                ...
                "resume": (
                    candidate_profile.resume.url if candidate_profile.resume else None
                ),
                "profile": (
                    candidate_profile.profile.url if candidate_profile.profile else None
                ),
                "reviews": review_list,
            }
    ...
    def delete(self, request):
        try:
            ...
            if job_post.approval_status == "accepted":
                return Response(
                    {"error": "Job is approved by manager, unable to delete this job"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        try:
            ...
            if organization_id:
                try:
                    ...
                    all_postings = JobPostings.objects.filter(
                        organization=organization, username=request.user
                    )
                    total_postings = all_postings.count()
                    completed = JobPostings.objects.filter(
                        organization=organization,
                        status="closed",
                        username=request.user,
                    ).count()
                    ...
                    for job in all_postings:
                        ...
                        processing = job_applications.filter(
                            status="processing"
                        ).count()
                    ...
                    return Response(
                        {"error": "Job not found or not authorized."}, status=404
                    )
            else:
                ...
                for job_item in job_items:
                    org = Organization.objects.filter(
                        manager=job_item.organization.manager
                    ).first()
...
class InterviewerRoundsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if not interviewer_id:
                return Response(
                    {"error": "Please send the interviewer id"},
                    status=status.HTTP_200_OK,
                )

            interviews = InterviewerDetails.objects.filter(
                name__id=interviewer_id
            ).select_related("job_id")

            remaining_interviewers = ClientDetails.objects.get(
                username=request.user
            ).interviewers.all()
            interviewers_list = []
            ...
            for interview in interviews:
                interviews_pending = InterviewSchedule.objects.filter(
                    interviewer=interview, status="pending"
                ).count()
                interview_list = {
                    "round_num": interview.round_num,
            ...
            return Response(
                {
                    ...
                    "remaining_interviewers": interviewers_list,
                },
                status=status.HTTP_200_OK,
            )
    ...
    def get(self, request):
        try:
            ...
            candidates = JobApplication.objects.filter(
                job_location__job_id__in=job_posts
            )
            ...
            for candidate in candidates:
                selectCand = (
                    SelectedCandidates.objects.filter(application=candidate)
                    .exclude(edit_request="")
                    .first()
                )
...
class FullJobDraftView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                FullJobDraftSerializer(draft).data, status=status.HTTP_201_CREATED
            )
    ...
    def patch(self, request, pk):
        ...
        if serializer.is_valid():
            draft = serializer.save()
            return Response(
                FullJobDraftSerializer(draft).data, status=status.HTTP_200_OK
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ContinueDraftView(APIView):
    def get(self, request):
        try:
            connection_id = request.GET.get("connection_id")
            organization = ClientOrganizations.objects.get(
                id=connection_id
            ).organization
            user = request.user
            try:
                job_draft = JobPostingDraftVersion.objects.get(
                    username=request.user, organization=organization
                )
            except JobPostingDraftVersion.DoesNotExist():
                return Response(
                    {"error": "Draft doesnot exists"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class CreateNewDraftView(APIView):
    def post(self, request):
        try:
            ...
            JobPostingDraftVersion.objects.create(
                username=request.user, organization=organization
            )
            return Response(
                {"message": "Draft created successfully"}, status=status.HTTP_200_OK
            )
...
class SkillSetDraftView(APIView):
    def post(self, request):
        try:
            ...
            try:
                job_draft = JobPostingDraftVersion.objects.get(
                    username=user, organization=organization
                )
            except JobPostingDraftVersion.DoesNotExist:
                job_draft = JobPostingDraftVersion.objects.create(
                    username=user, organization=organization
                )

            incoming_skill_names = [skill.get("skill_name") for skill in data]
            SkillMetricsDraftVersion.objects.filter(
                job=job_draft, is_primary=is_primary
            ).exclude(skill_name__in=incoming_skill_names).delete()
            ...
            for skill in data:
                ...
                skill_obj, created = SkillMetricsDraftVersion.objects.update_or_create(
                    ...
                    defaults={"metric_type": metric_type, "metric_value": metric_value},
                )
            ...
            return Response(
                {"message": "Skills saved as draft successfully."},
                status=status.HTTP_200_OK,
            )
    ...
    def post(self, request):
        try:
            ...
            JobLocationsDraftVersion.objects.filter(
                job=job_draft,
            ).exclude(location__in=incoming_location_names).delete()
            ...
            for location in data:
                ...
                JobLocationsDraftVersion.objects.update_or_create(
                    ...
                    defaults={"job_type": job_type, "positions": positions},
                )
...
class InterviewersDraftView(APIView):
    def post(self, request):
        try:
            ...
            for round_data in data:
                ...
                try:
                    user_obj = CustomUser.objects.get(
                        id=interviewer_id, role="interviewer"
                    )
                except CustomUser.DoesNotExist:
                    continue

                round_obj = InterviewerDetailsDraftVersion.objects.filter(
                    job=job_draft, round_num=round_num
                ).first()
                if round_obj:
            ...
            return Response(
                {"message": "Draft saved successfully"}, status=status.HTTP_200_OK
            )
...
class DeleteNegotiation(APIView):
    def delete(self, request):
        try:
            ...
            return Response(
                {"message": "Negotiations deleted successfully"},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            print("Error:", str(e))
...
class ApproveDeadline(APIView):
    ...
    def put(self, request, job_id):
        try:
            ...
            return Response(
                {"message": "Deadline extended successfully"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            print("Error:", str(e))
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
